...
def get_correlations_for_subjects(subjects, task_getter, model, predictions=None):
    ...
    for task_index, task in enumerate(tasks):
        ...
        predicted_by_subj[subjects[0]][:] = 0
        ...
        #                                            subjects_actual_maps=actual_by_subj, save=False)
        print("iou positive with predictions: {0:.3f}, iou positive with canonical: {1:.3f}".format(np.mean(iou_positive), np.mean(iou_positive)))
        print("iou negative with predictions: {0:.3f}, iou negative with canonical: {1:.3f}".format(np.mean(iou_negative), np.mean(iou_positive)))
...
def run_evaluation(model):
    ...
    get_correlations_for_subjects(subjects_test, tasks_getter, model, predictions)
